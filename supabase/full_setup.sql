...
begin
  ...
  values (new.id, coalesce(new.email, new.id::text || '@civicsync.local'), 'citizen')
...
begin
  if not exists (
    select 1 from pg_policies where schemaname='storage' and tablename='objects' and policyname='objects_select_public_civicsync'
  ) then
    create policy objects_select_public_civicsync
    on storage.objects
    for select
  ...
  if not exists (
    select 1 from pg_policies where schemaname='storage' and tablename='objects' and policyname='objects_insert_public_civicsync'
  ) then
    create policy objects_insert_public_civicsync
    on storage.objects
    for insert
  ...
  if not exists (
    select 1 from pg_policies where schemaname='storage' and tablename='objects' and policyname='objects_update_public_civicsync'
  ) then
    create policy objects_update_public_civicsync
    on storage.objects
    for update
    using (bucket_id in ('issue-images', 'resolution-proofs'))
    with check (bucket_id in ('issue-images', 'resolution-proofs'));
  ...
  if not exists (
    select 1 from pg_policies where schemaname='storage' and tablename='objects' and policyname='objects_delete_public_civicsync'
  ) then
    create policy objects_delete_public_civicsync
    on storage.objects
    for delete
    using (bucket_id in ('issue-images', 'resolution-proofs'));
  end if;
