...
export function IssueMap() {
  ...
  return (
    <div className="space-y-2">
      <div
        className="inline-flex rounded-md border p-1 text-xs font-medium"
        style={{ borderColor: "var(--border)", background: "var(--bg)", color: "var(--text)" }}
      >
        <button
          type="button"
          onClick={() => setMapMode("marker")}
          className={`rounded px-3 py-1.5 transition ${mapMode === "marker" ? "" : ""}`}
          style={
            mapMode === "marker"
        ...
        <button
          ...
          className={`rounded px-3 py-1.5 transition ${mapMode === "heatmap" ? "" : ""}`}
          style={
            mapMode === "heatmap"
              ? { background: "var(--primary)", color: "var(--on-primary)" }
              : { color: "var(--text)" }
      ...
      <div
        ref={mapContainerRef}
        className="h-[65vh] min-h-[360px] w-full overflow-hidden rounded-xl border"
        style={{ borderColor: "var(--border)", background: "color-mix(in srgb, var(--surface) 72%, var(--bg))", touchAction: "pan-x pan-y" }}
      />
      <p className="text-xs text-muted">{statusText}</p>
      <div className="flex flex-wrap gap-2 text-xs" style={{ color: "var(--text)" }}>
        <span className="rounded-full px-2 py-1" style={{ background: "color-mix(in srgb, #ef4444 24%, var(--bg))", border: "1px solid color-mix(in srgb, #ef4444 45%, transparent)" }}>Red: Unresolved</span>
        <span className="rounded-full px-2 py-1" style={{ background: "color-mix(in srgb, #eab308 24%, var(--bg))", border: "1px solid color-mix(in srgb, #eab308 45%, transparent)" }}>Yellow: In Progress</span>
        <span className="rounded-full px-2 py-1" style={{ background: "color-mix(in srgb, #22c55e 24%, var(--bg))", border: "1px solid color-mix(in srgb, #22c55e 45%, transparent)" }}>Green: Resolved</span>
      </div>
    </div>
  );
}
