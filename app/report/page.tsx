...
export default function ReportPage() {
  ...
  useEffect(() => {
    const requireLogin = async () => {
      ...
      if (!user) {
        router.replace("/citizen/login?next=/report");
        return;
      }
  ...
  const supportExistingIssue = async (candidateIssueId: string) => {
    try {
      const supabase = getSupabaseBrowserClientOrNull();
      if (!supabase) {
      ...
      if (!profile?.id) {
        throw new Error("Please log in before supporting an issue.");
      }

      const { error: upvoteError } = await supabase
        .from("upvotes")
        .upsert({ issue_id: candidateIssueId, user_id: profile.id }, { onConflict: "issue_id,user_id", ignoreDuplicates: true });
      ...
      await createIssueEvent(supabase, {
        ...
        eventType: "upvote",
        message: "Citizen supported existing issue from duplicate check",
        createdBy: "citizen",
      });

      setStatusMessage("You supported an existing nearby issue. Redirecting to details...");
      router.push(`/issue/${candidateIssueId}`);
    } catch (caughtError) {
      setStatusMessage(caughtError instanceof Error ? caughtError.message : "Could not support existing issue.");
    }
  };
  ...
  return (
    <section className="mx-auto w-full max-w-md space-y-4">
      <div className="space-y-1">
        <h1 className="text-2xl font-semibold tracking-tight" style={{ color: "var(--text)" }}>Report an Issue</h1>
        <p className="mt-1 text-sm text-muted">Capture the issue, confirm location, and submit.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="surface-card space-y-4 p-4 sm:p-5">
        ...
        <div className="space-y-1.5">
          <label htmlFor="image" className="text-sm font-medium" style={{ color: "var(--text)" }}>
            Issue Photo
          </label>
          <input
            id="image"
            ...
            required
          />
          <p className="text-xs text-muted">Camera opens on mobile, and gallery upload is also supported.</p>
          {imagePreviewUrl ? (
            <img src={imagePreviewUrl} alt="Issue preview" className="mt-2 h-40 w-full rounded-md border object-cover" style={{ borderColor: "var(--border)" }} />
          ) : null}
        </div>

        <div className="space-y-2">
          <div className="flex items-center justify-between gap-2">
            <p className="text-sm font-medium" style={{ color: "var(--text)" }}>Detected Location</p>
            <button
              type="button"
            ...
            {location ? (
              <>
                {miniMapUrl ? (
                  ...
                  <div
                    className="flex h-36 items-center justify-center rounded-md px-2 text-xs text-center"
                    style={{ background: "color-mix(in srgb, var(--bg) 84%, var(--surface))", color: "var(--muted)" }}
                  >
        ...
        {duplicateCandidates.length > 0 ? (
          <div
            ...
            style={{ borderColor: "var(--border)", background: "color-mix(in srgb, var(--accent) 18%, var(--surface))" }}
          >
            <p className="text-sm font-semibold" style={{ color: "var(--text)" }}>Potential duplicate reports detected nearby</p>
            ...
            <ul className="mt-2 space-y-2">
              {duplicateCandidates.map((candidate) => (
                <li
                  ...
                  style={{ borderColor: "var(--border)", background: "color-mix(in srgb, var(--bg) 90%, var(--surface))" }}
                >
                  <p className="text-sm font-medium" style={{ color: "var(--text)" }}>{candidate.issue.title}</p>
                  <p className="mt-1 text-xs" style={{ color: "var(--muted)" }}>
                  ...
                  <button
                    type="button"
                    className="btn-secondary mt-2 ml-2 px-2.5 py-1 text-xs"
                    onClick={() => void supportExistingIssue(candidate.issue.id)}
                  >
                    Support Existing
                  </button>
                </li>
            ...
            {!confirmedDuplicateOverride ? (
              ...
              <p className="mt-3 text-xs font-medium" style={{ color: "var(--text)" }}>Duplicate warning acknowledged. Submit to continue.</p>
            )}
          </div>
        ) : null}

        {statusMessage ? <p className="text-sm" style={{ color: "var(--text)" }}>{statusMessage}</p> : null}
